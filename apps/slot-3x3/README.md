...
The app lives in [apps/slot-3x3](.). It follows the existing Web SDK slot-game
patterns from `apps/lines`, but keeps the implementation smaller and focused on
this game.
...
- The current code defines 5 paylines in
  [constants.ts](src/game/constants.ts): top row, middle row, bottom row,
  diagonal down, and diagonal up.
...
- There are no 2-symbol wins because the paytable only defines 3-symbol payouts
  in [config.ts](src/game/config.ts).
